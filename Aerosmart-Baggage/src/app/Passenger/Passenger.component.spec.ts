...
describe('PassengerComponent', () => {
  ...
  it('should update the table when a Passenger is added', fakeAsync(() => {
    ...
    sinon.stub(component.servicePassenger, 'addParticipant').returns(new Observable(observer => {
      observer.next('');
      observer.complete();
    }));
  ...
  it('should update the table when a Passenger is updated', fakeAsync(() => {
    let loadAllSpy = sinon.stub(component, 'loadAll');
    sinon.stub(component.servicePassenger, 'updateParticipant').returns(new Observable(observer => {
      observer.next('');
      observer.complete();
    }));
  ...
  it('should update the table when a Passenger is deleted', fakeAsync(() => {
    let loadAllSpy = sinon.stub(component, 'loadAll');
    sinon.stub(component.servicePassenger, 'deleteParticipant').returns(new Observable(observer => {
      observer.next('');
      observer.complete();
    }));
